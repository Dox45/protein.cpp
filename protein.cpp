        ...
        int            seq_len)
{
    ...
    for (int li = 0; li < hp.n_layers; li++) {
        ...
        if (dbg) {
            fprintf(stderr, "[debug] K ne: [%ld, %ld, %ld, %ld]\n", K->ne[0], K->ne[1], K->ne[2], K->ne[3]);
            fprintf(stderr, "[debug] Q ne: [%ld, %ld, %ld, %ld]\n", Q->ne[0], Q->ne[1], Q->ne[2], Q->ne[3]);
        }
        ...
        if (dbg) {
            fprintf(stderr, "[debug] V_perm ne: [%ld, %ld, %ld, %ld]\n", V_perm->ne[0], V_perm->ne[1], V_perm->ne[2], V_perm->ne[3]);
